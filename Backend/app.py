...
def edit_task(task_id):
    ...
    with get_db() as conn:
        ...
        conn.execute(
            """
            INSERT INTO inbox (user_id, project_id, task_id, message)
            SELECT pm.user_id, pm.project_id, ?, ?
            FROM project_member pm
            JOIN project_task pt ON pm.project_id = pt.project_id
            WHERE pt.task_id = ?
            """,
            (task_id, f"Task was updated", task_id)
        )
...
def add_project_member(project_id):
    ...
    with get_db() as conn:
        owner = conn.execute(
            "SELECT 1 FROM project_member WHERE project_id = ? AND user_id = ? AND role = 'owner'",
        ...
        try:
            ...
            conn.execute(
                "INSERT INTO inbox (user_id, project_id, message) VALUES (?, ?, ?)",
                (user["user_id"], project_id, f"You were added to project {project_id}")
            )
...
def assign_task(project_id):
    ...
    with get_db() as conn:
        ...
        try:
            ...
            conn.execute(
                "INSERT INTO inbox (user_id, project_id, task_id, message) VALUES (?, ?, ?, ?)",
                (assigned_to, project_id, task_id, f"You were assigned a task")
            )
...
def get_project_tasks(project_id):
    ...
    with get_db() as conn:
        ...
        tasks = conn.execute(
            """
            SELECT task.*, project_task.assigned_to
            FROM task
...
def get_inbox():
    ...
    with get_db() as conn:
        notes = conn.execute(
            ...
            FROM inbox
            LEFT JOIN project ON inbox.project_id = project.project_id
            LEFT JOIN task ON inbox.task_id = task.task_id
            WHERE inbox.user_id = ?
...
@app.route("/inbox/<int:id>/read", methods=["PATCH"])
def mark_inbox_read(id):
    auth_error = login_required()
    if auth_error:
        return auth_error
    ...
    with get_db() as conn:
        conn.execute(
            "UPDATE inbox SET is_read = 1 WHERE id = ? AND user_id = ?",
            (id, session["user_id"])
        )

    return jsonify({"message": "Inbox item marked as read"}), 200
